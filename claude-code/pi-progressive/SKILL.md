...
| 六·固防 | 修复/新功能/重构后 + 加防回归 + **方向性测试检查**：检查现有测试覆盖→缺失测试暴露→测试方向建议→回归风险标注→新功能测试计划→重构行为变化检测 |
...
4. 检查**测试文件**是否覆盖了修改后的行为——触发方向性测试评估(§4.1·固防)：现有测试是否覆盖变更的边界？缺失则列出测试缺口
...
| 🔎核验 | 确认修复完整，无残留副作用 |
| 🔲边界 | 覆盖全部边界条件 + 方向性测试评估(§4.1·固防)：测试覆盖缺口已暴露并声明 |
| 🧭校准 | 校准场景匹配 |
| 📏正名 | 校验命名与业务一致 |
| ⭐极致 | 确认最优解，无可再优 |
