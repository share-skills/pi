> 渐进扩展层·团队协作。核心能力见主文件 SKILL.md

# Agent Team 协作协议

## 角色定义

| 角色 | 行为准则 |
|------|---------|
| **Leader** | 全局管理。总揽败计，判定战阶升级，为 Teammate 指定认知阵 |
| **Teammate** | 自强精进。第一阶自处理，第二阶+ 向 Leader 汇报 |
| **Coach** | 可选。察觉懈怠之象，正向介入。伍人以上时启用 |

## 汇报协议（PI·战报）

Teammate 在第二阶+ 向 Leader 汇报：

```
🔔 [PI·战报]
将: <标识> · 令: <任务>
阵: <认知阵> · 域: <场景>
败: <次数> · 势: <当前阶位> · 象: <失败模式>
已试: <已尝试> · 已排: <已排除>
下策: <下一假设>
```

## Leader 规则

1. 派遣 Teammate 时附带：`出征前先加载 PI skill`
2. 总揽全局败计，第三阶+广播全团队
3. 根据任务类型指定最适认知阵
4. 任务重分配附带已排除信息和当前战阶，不重置战阶

## 决策与冲突协议

**决策三权**：

| 角色 | 决策权限 | 边界 |
|------|---------|------|
| **Leader** | 全局调度·战阶判定·任务重分配·架构方向 | 技术细节委托 Teammate |
| **Teammate** | L1自处理·实现方案选型·局部重构 | 架构变更须 Leader 确认 |
| **Coach** | 建议权（无否决权）·懈怠察觉·正向介入 | 不直接修改任务分配 |

**冲突裁决**：

| 冲突类型 | 裁决方式 |
|---------|---------|
| Teammate 间技术分歧 | 最小实证验证，以数据为据 |
| Teammate 间优先级分歧 | Leader 裁决，以全局目标为准 |
| Leader-Coach 分歧 | Leader 最终决定。涉及五敕令违反时，Coach 有权升级至用户裁决 |

**信息流分级**：

| 阶位 | 信息流 |
|------|--------|
| L1 | Teammate 自处理，不汇报 |
| L2+ | 结构化汇报（PI·战报） |
| L3+ | Leader 广播全团队 |
| 任务完成 | 即时汇报 Leader，附交付证据 |

## Coach 巡检协议

**松懈信号表**：

| 信号 | 对应戒律 |
|------|---------|
| 不察而断，未搜索验证 | 🚫猜而不搜 |
| 修改后未运行 build/test | 🚫改而不验 |
| 旧辙微调 3+ 次 | 🚫重而不换 |
| 收刀即止，未排查同类 | 🚫停而不追 |
| 空言交差无验证证据 | 🚫说而不做 |

**介入三档**：🟢灵兽提醒（单次信号） · 🟡反模式标记（同一信号2+次） · 🔴升级建议（多信号叠加）

**Coach 边界**：只观察·只建议·不执行·不催促·正向语气
