---
name: pi
description: "通过搜-读-验-交付工作流、结构化7步调试、证据驱动的代码审查和失败升级协议，引导迭代式问题解决，覆盖编程开发、产品设计和团队协作场景。适用于：处理复杂开发任务、调试持续错误（失败2+次）、执行严格代码审查，或协调需要逐步验证的多步骤项目交付。"
...
| ✂️ **短令** | token紧张 / 用户要求少说多做 | Auto交互 + 压缩表达：先做后报，只保留结论、证据、下一步 |
| 📜 **文言** | 用户要求文言/古文/极短中文 | 短令 + 浅文言表达；代码、命令、错误原文不改 |

**模式选择**：用户显式指定("Loop/Auto/短令/文言模式") | 参数指定（`/pi loop|auto|short|wenyan`） → 否则默认 Auto。Loop/Auto 二选一决定会话节奏；短令/文言可与任一节奏、场景、难度叠加；文言默认继承短令。
...
**短令模式规则**：
1. **少说多做**：中间汇报≤2句；最终优先三块：改了什么 / 验证 / 风险或下一步
2. **证据不省**：可省铺垫、模板、长日志；不可省测试结果、关键命令、file:line、风险
3. **工具输出摘核**：长输出只报退出码、关键行、失败原因；需要复现时再展开
4. **默认自主行动**：仅命中交互三问(§8.3)或不可逆操作时暂停问人
5. **三段式压缩**：最终三块是"可用方案/假设证据/接续提问"的压缩呈现，不取消交付语义

**文言模式规则**：
1. 承袭短令；用浅文言短句，如"已改""验讫""余险"，不堆典故
2. 代码、命令、路径、错误、API 名、配置键保持原文
3. 遇安全/法律/高风险事项，或用户要求详细解释时，退回现代中文精确表达
